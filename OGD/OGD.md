...
<a name="ogd-top"></a>
# Organic Game Design (OGD) `v0.9.6-beta`
...
- [Базовая Аксиома](#ogd-axiom)
  ...
  - [Что игрок берёт из игры](#ogd-reading-rule)
...
<a name="ogd-reading-rule"></a>
## Что игрок берёт из игры

[Базовая Аксиома](#ogd-axiom) говорит простую вещь: человек тянется в игру от собственного дефицита, а игру саму по себе нельзя назвать «весёлой» в отрыве от того, кому она сейчас нужна. У этого тезиса есть прикладное следствие, которое стоит проговорить в самом начале — оно задаёт тон всем дальнейшим разделам метода.

Один и тот же игровой материал разные игроки используют по-разному, исходя из того, что у них болит. Игра — это материал, а регуляция — это работа игрока с этим материалом. Автор создаёт лишь *возможность* опыта. Финальную форму этой возможности придаёт игрок своим состоянием и своим запросом.

Чтобы говорить об этом точно, введём один термин.

**Прочтение** — это то, как игрок фактически использует игру: какую жизненную проблему он через неё лечит и каким игровым действием он эту проблему закрывает. Прочтение не принадлежит ни игре, ни игроку по отдельности — оно возникает в отношении между ними.

Дизайнер всегда проектирует своё **целевое прочтение**: гипотезу о том, какому игроку, с каким дефицитом и какое действие он собирается предложить. Это его явный выбор, и вокруг него строится вся игра и весь маркетинг. Но финальное прочтение всегда остаётся за игроком.

> *Пример:* Cookie Clicker задумывался как тихая игра для усталого человека. Цифры медленно растут, тревога за будущее отступает, ничего не требует быстрых решений. Это его целевое прочтение. Но часть игроков приходит в ту же самую игру с другим запросом: от скуки, от отсутствия интеллектуального вызова. Они превращают её в задачу оптимизации — как эффективнее купить апгрейды, какой порядок действий выжимает максимум. Это параллельное прочтение той же игры. Ни одно не отменяет другое, пока эти две группы игроков не начинают мешать друг другу.

На практике из этой идеи следуют три обязательных шага, через которые дизайнер проходит на каждом проекте.

Первый шаг — назвать целевое прочтение явно и конкретно. Общие слова вроде «кози-игра для расслабления» не работают. Дизайнер должен ответить на три вопроса: какого игрока он видит, какая проблема у этого игрока в жизни, какое игровое действие эту проблему закрывает. Точный язык для ответов метод даст ниже — в разделах [Пять сфер жизни игрока](#ogd-spheres) (о том, где у игрока бывает дефицит) и [Агентность и инструменты OGD](#ogd-agency-tools) (о том, какими действиями этот дефицит закрывается). Но само требование конкретики действует с самого начала: без него дизайнер не может проверить собственную гипотезу.

Второй шаг — признать, что целевое прочтение не единственное. Игра с достаточной глубиной системы всегда допускает альтернативы. Оптимизаторы в тихой игре — это нормальная работа Аксиомы. Паниковать бесполезно, запрещать невозможно.

Третий шаг — проверить, не разрушают ли побочные прочтения целевое. Если активное меньшинство начинает давить на массу («играть надо только так, это единственный правильный способ»), целевое прочтение рушится для всех остальных. За совместимостью прочтений следят через интерфейс, туториал, маркетинг и коммуникацию с сообществом.

Из этой идеи не следует, что игру можно делать «сразу для всех». Попытка обслужить два противоположных запроса одновременно — отдельный риск, с ним метод работает дальше, в разделе [Базовые типы игр](#ogd-game-types), где вводятся понятия Игры-Маятника и Игры-Буфета. Речь здесь о другом: в любую хорошо спроектированную игру естественно притягивается побочная аудитория с иным запросом, и с этим фактом нужно уметь жить, не разрушая целевое прочтение.
...
> [!NOTE]
> Четыре типа ниже описывают **целевое прочтение** игры — то прочтение, ради которого дизайнер строит механики и маркетинг. Само игровое содержимое не «является» убежищем или тренажёром в вакууме; тип возникает из того, каким прочтением дизайнер наполняет материал. Подробнее см. раздел [Что игрок берёт из игры](#ogd-reading-rule).
...
Выше описаны несколько типов игр и их общая логика. Чтобы понять, какое целевое прочтение дизайнер заложил в игру, достаточно посмотреть на её игровые механики и сюжет/нарратив. Каждая механика либо развеивает скуку (избыточный порядок), либо успокаивает тревогу (избыточный хаос). 
...
> [!IMPORTANT]
> 💡 **Правило**: Если основные игровые механики задуманы стабилизировать тревогу и стресс игрока — дизайнер целится в Убежище. Если стимулировать скуку и застой в жизни — в Тренажёр. Это выбор *целевого прочтения* (см. раздел [Что игрок берёт из игры](#ogd-reading-rule)).
...
**Стабилизатор** хаоса — глагол ЗАКРЫТЬСЯ. Изоляция, стены, укрытия, барьеры. Примеры: Minecraft (строительство базы), стены и убежища в играх на выживание.
...
> [!NOTE]
> **Два режима одного инструмента часто живут в одной игре одновременно.**
>
> Инструмент Эффективности чаще других допускает параллельные прочтения. Основная аудитория активирует Упростить — запустила idle-механику, смотрит на рост чисел, получает стабилизацию Ресурсов. Меньшинство-оптимизаторы активируют Усложнить на том же материале — выжимают максимум из префестижа и множителей, получают стимуляцию Познания. Обе работы реальны и могут сосуществовать, если не блокируют друг друга. Это классический случай принципа [Что игрок берёт из игры](#ogd-reading-rule).
>
> Остальные Инструменты тоже допускают параллельные прочтения, но там для этого нужна сознательная дизайнерская глубина системы. Minecraft позволяет и Закрыться (строить базу), и Открыться (уходить в дальние экспедиции). Stardew Valley даёт и Сблизиться (тёплые отношения с NPC), и Манипулировать (оптимизация сердец как ресурса). DOOM задумана как Подавить, но спидраннерское сообщество превращает её в Бросить вызов. Эффективность особенная тем, что её два режима возникают почти автоматически: Упростить и Усложнить живут в одной и той же механике, им достаточно лишь смещения цели в голове игрока.
...
> [!NOTE]
> Правая колонка «Примечания / когда иначе» описывает альтернативные прочтения — случаи, когда одна и та же механика при другой подаче решает задачу в другой Сфере или в другом режиме. Эти оговорки — частные случаи общего принципа из раздела [Что игрок берёт из игры](#ogd-reading-rule).
...
| Стены, оборона периметра, статичная база | защищаться, удерживать | **Закрыться** | Среда + Сила | Стабилизатор | Про укрытие и удержание рубежа стенами (база в играх на выживание). Если требует высокого темпа/стресса — уходит в Подавить. |
| Tower defense: расстановка башен против волн | планировать, просчитывать | **Разобрать + Упростить** | Познание + Ресурсы | Стимулятор | Ядро — просчёт расстановки и связок башен; оборона/удержание здесь лишь форма поля, не само переживание. Если игрок сам бьётся в гуще — Подавить; расслабленный фоновый режим — разгрузка. |
...
| Боссы/серия испытаний (“коридор” челленджей) | пройти, победить, превозмочь | **Бросить вызов** | Сила | Стимулятор | Когда нужно выучить паттерн босса — часто смешивается с Разобрать. |
...
| Фарм/ферма/полив/рутина | выращивать, ухаживать | **Закрыться** (иногда Упростить) | Среда + Ресурсы | Стабилизатор | Держит принадлежность к месту, в которое возвращаешься (Закрыться); у оптимизаторов рост хозяйства как процесс (Упростить). «Собрать» (упорядочивание) это слабый вторичный слой. |
...
| Перемещение как медитация (ходьба/драйвинг‑сим) | ехать, идти, быть в пути | **Закрыться** (иногда Открыться) | Среда | Стабилизатор | Здесь «Закрыться» — отгородиться от внешнего мира (даже в открытом пространстве), а не буквальные стены. Если путь про новизну и риск (шторм/угрозы) — Открыться/Обойти. |
| Хоррор: неизвестность и угроза среды | бояться, выживать | **Открыться** (как опасная неизвестность) | Среда | Стимулятор | Если главный глагол — “выкрутиться” без боя, добавляется Обойти; в хорроре на выживание с укрытиями (прятки) — Закрыться. Часть ценности хоррора лежит за рамкой регуляции (см. [Где OGD не работает](#ogd-eudaimonic-boundary)). |
| Таймеры, дедлайны, гонка со временем | успеть, выдержать темп | **Бросить вызов** | Сила (+ Познание) | Стимулятор | Таймер — регулятор интенсивности: на высокой ставке даёт вызов, и через пик напряжения игрок входит в поток («стресс и медитация одновременно») — одна петля с переключателем интенсивности. Мягкий таймер, учащий оптимизации, смещается в Упростить (Ресурсы). |
...
<a name="ogd-dev-trust"></a>
### Доверие к разработчику (фактор LTV вне дизайна опыта)

Отдельно — **доверие к разработчику** как фактор LTV за рамкой дизайна опыта. Предательство игрока на дистанции (брошенный ранний доступ, ухудшение купленного патчем, невыданный обещанный контент) почти не трогает опыт в игре, но мощно отравляет рекомендацию: «игра хорошая, но не советую — из-за того, что сделал разработчик». По амплитуде способно обрушить оценку даже хорошей игры, хотя обычно обратимо, если разработчик исправляется. Как этого избегать — вопрос релиз-стратегии и работы с сообществом, вне рамки OGD; метод лишь фиксирует, что доверие к автору — часть LTV, которую дизайн опыта не покрывает.
...
> Исключение: сюжетные игры — но здесь две разные механики.
>
> Нарратив-загадка (детектив, твист) закрывает **дефицит информации**: узнал разгадку — значимость после титров падает почти до нуля, перепроходить незачем. Это «эффект саранчи», хвост короткий. Массовая аудитория сюда заходит редко — разве что хиты уровня игр-кино от Sony.
>
> Нарратив со **стойким эмоциональным следом** (горе, прощание, экзистенциальный сдвиг) дефицит не закрывает — опыт работает и после титров. Значимость закрепляется и идёт в сарафан: «рыдал в финале», «помогла пережить потерю», «думаю о ней спустя годы». Хвост длинный — *Spiritfarer* и *To the Moon* продаются годами. Для этого класса вывод про короткий хвост неверен (см. [Где OGD не работает](#ogd-where-not-work)).
> 
...
> [!NOTE]
> **Сеттинг не диктует тип игры.** Один и тот же сеттинг может быть Убежищем, Тренажёром или Буфетом — всё зависит от целевого прочтения, выбранного дизайнером (см. [Что игрок берёт из игры](#ogd-reading-rule)). Космос бывает уютным (Astroneer) и смертельным (Dead Space). Фэнтези бывает расслабляющим (Ni no Kuni, Dragon Quest Builders) и жестоким (Elden Ring, Dark Souls).
>
...
| **Tower Defense** | Познание + Ресурсы | Разобрать + Упростить | Стимулятор | **Тренажёр.** Решить карту просчётом. Расслабленный режим → Убежище; фаза стройки и исследования → Маятник; активный бой героем → Подавить. |
| **Survival** | Ресурсы + Среда | Обойти + Закрыться | Стим → Стаб | **Маятник.** Стресс в начале, комфорт базы потом. |
| **Simulators / Farm** | Среда + Ресурсы | Закрыться + Упростить | Стабилизатор | **Убежище.** Чистое. |
...
> [!NOTE]
> Колонка *«Типичный тип игры»* описывает типичное целевое прочтение жанра — то, под какое прочтение обычно проектируют игры этого жанра. Конкретный проект может целиться в нетипичное прочтение: *Strategy* без угроз становится Убежищем, *Puzzle* может быть и расслабляющим, и хардкорным, *Shooter* с casual-режимами сдвигается в Маятник. См. [Что игрок берёт из игры](#ogd-reading-rule).
...
2.  **Выпиши обещание Жанра:** Какой Инструмент он обещает? (Например, *Farm Sim* = Закрыться/Упростить, свой уголок и рутина).
...
Тип, который ты выбираешь здесь — это твоё *целевое прочтение* (см. [Что игрок берёт из игры](#ogd-reading-rule)). Цель шага — прогноз коммуникации и рисков: чем сложнее тип, тем важнее “честность ярлыков” и транзит.
...
**Шаг 4.1 Проверь прочтения на конфликт**

Есть отдельный тип поломки, который не ловится сверкой маркетинга с геймплеем. Иногда маркетинг и геймплей согласованы между собой, но массовая аудитория всё равно читает игру иначе, чем задумывал дизайнер. Типичный случай: вокруг кози-игры образовалось сообщество оптимизаторов, и его гайды начали давить на массу. Массовая аудитория чувствует, что «играет неправильно», и отваливается — хотя ни маркетинг, ни механики её специально не отталкивают.

Сформулируй два прочтения и сравни их: какое ты закладывал в дизайн (целевое), и какое фактически активирует массовая аудитория? Если они расходятся и активное меньшинство давит на целевое прочтение, это отдельный диагноз, и лечится он через коммуникацию с сообществом, а не через правку механик. Подробнее см. [Что игрок берёт из игры](#ogd-reading-rule).
...
2. *Индивидуальные различия*. Один и тот же человек в разные дни может искать разное, и разные люди в одной аудитории могут активировать разные прочтения одной и той же игры. Концепция прочтения (см. [Что игрок берёт из игры](#ogd-reading-rule)) частично покрывает эту проблему. Но предсказательная сила метода остаётся на уровне аудиторий; поведение отдельного игрока требует других инструментов — плейтестов, аналитики, исследований.
...
<a name="ogd-eudaimonic-boundary"></a>
### За пределами рамки: эвдемоническая ценность

Пять состояний выше — сбои метода на **своей** территории: игрок ищет регуляцию, но что-то искажает выбор. Эвдемоническая ценность — не сбой, а другая территория.

OGD описывает игру как инструмент **регуляции** (возврат к балансу) — это гедоническая ценность, «мне стало лучше». Но есть второй, независимый мотив — **эвдемонический**: игрок ищет не облегчения, а смысла и стойкого следа. Иногда ценность именно в том, что баланс НЕ восстановлен: горе прожито, но не снято; вопрос задан, но не закрыт. Высшую оценку ставят не вопреки дискомфорту, а из-за него. Мотив концентрируется в узком классе нарративных игр (*Spiritfarer*, *To the Moon*, отчасти *SOMA*) и почти отсутствует в геймплейных. Его инструменты (Мягкая Посадка, Правило Пик-Конец) тут не работают — они ждут позитивной оценки на выдохе, а она рождается из неснятого напряжения.

> [!NOTE]
> **Научная опора (вне ядра OGD).** Гедоническая vs эвдемоническая мотивация потребления медиа: Oliver & Raney (2011) — два различимых, во многом независимых измерения, а не полюса одного континуума. На играх: Bopp, Mekler & Opwis (2016) — игроки ценят и приветствуют негативные эмоции. Ценность без разрядки/катарсиса: Menninghaus et al. (2017), distancing-embracing model. Эвдемония в русле SDT-традиции: Huta & Ryan (2010).

Ещё одна область за рамкой метода — **доверие к разработчику** как фактор LTV вне дизайна опыта: предательство игрока на дистанции (заброс, нерф купленного) отравляет рекомендацию, не трогая опыт в игре. Задокументирована отдельно — см. [Доверие к разработчику](#ogd-dev-trust).
