...
> [!TIP]
> Тип, который ты определяешь здесь — это *целевое прочтение*, то, под которое ты строишь игру. Часть игроков может прочитать её иначе, и это нормально. См. [Что игрок берёт из игры](OGD.md#ogd-reading-rule).
...
**Разобраться, почему одну игру разные игроки читают по-разному:**
→ Раздел [Что игрок берёт из игры](OGD.md#ogd-reading-rule)

**Подобрать механики под конкретную задачу:**
→ Раздел [Агентность и инструменты](OGD.md#ogd-agency-tools)

**Проверить, не противоречит ли сенсорика обещанию игры:**
→ Раздел [Сенсорный слой и Game Feel](OGD.md#ogd-sensory-layer)

**Построить позиционирование через формулу «Сфера + Инструмент»:**
→ Раздел [Маркетинг Регуляции](OGD.md#ogd-marketing-regulation)

**Проверить, не конфликтуют ли жанр и сеттинг друг с другом:**
→ Раздел [Жанр и сеттинг](OGD.md#ogd-genre-setting)
