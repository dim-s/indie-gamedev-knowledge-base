...
- [Модель Игрового Гомеостаза](#модель-игрового-гомеостаза)
  - [Оглавление](#оглавление)
  - [Предисловие](#предисловие)
  - [На какие вопросы отвечает LHM?](#на-какие-вопросы-отвечает-lhm)
- [TL:DR (В двух словах о модели)](#tldr-в-двух-словах-о-модели)
    - [LHM - определение модели](#lhm---определение-модели)
...
- [Агентность и инструменты для нее](#агентность-и-инструменты-для-нее)
  ...
  - [Справочник игровых механик → глаголы LHM](#справочник-игровых-механик--глаголы-lhm)
  ...
  - [Справочник аудиторий по эпохам жизни](#справочник-аудиторий-по-эпохам-жизни)
    - [Дошкольники (примерно 3–6)](#дошкольники-примерно-36)
    - [Ранняя школа (примерно 6–10/11)](#ранняя-школа-примерно-61011)
    - [Поздняя школа / пубертат (примерно 12–17)](#поздняя-школа--пубертат-примерно-1217)
    - [Студенчество / колледж (примерно 18–24/25)](#студенчество--колледж-примерно-182425)
    - [Работа (примерно 25–44)](#работа-примерно-2544)
    - [Работа (примерно 45–60)](#работа-примерно-4560)
...
- [Нарратив и Сюжет](#нарратив-и-сюжет)
    ...
    - [Нарративные Инструменты LHM (Справочник)](#нарративные-инструменты-lhm-справочник)
    ...
    - [Гипотеза применимости LHM вне игр](#гипотеза-применимости-lhm-вне-игр)
...
- [Протокол применения LHM (практическая процедура)](#протокол-применения-lhm-практическая-процедура)
...
- [Где LHM не работает](#где-lhm-не-работает)
    - [Где LHM даёт сильный сбой и не работает:](#где-lhm-даёт-сильный-сбой-и-не-работает)
    - [Как маркетинг помогает преодолевать ограничения и сбои LHM](#как-маркетинг-помогает-преодолевать-ограничения-и-сбои-lhm)
- [Эпилог: Конкуренция за Агентность](#эпилог-конкуренция-за-агентность)
  - [Долгосрочная лояльность](#долгосрочная-лояльность)
- [Послесловие](#послесловие)
    - [Зачем создана LHM](#зачем-создана-lhm)
    - [Реальность как источник инноваций](#реальность-как-источник-инноваций)
    - [Контекст вместо чистого листа](#контекст-вместо-чистого-листа)
    - [Снижение трения вместо удержания](#снижение-трения-вместо-удержания)
    - [Компенсация вместо эскапизма](#компенсация-вместо-эскапизма)
    - [Удовлетворение вместо стимуляции](#удовлетворение-вместо-стимуляции)
...
Чтобы не говорить общими словами, LHM вводит Сферы — пять крупных областей жизни, где чаще всего возникает этот перекос. Для геймдизайнера это карта: какой запрос игрок приносит в игру и почему одна и та же механика одного «собирает», а другого оставляет равнодушным.

В основе здесь лежит теория ERG как удобная группировка потребностей. Но для проектирования нам нужна чуть более точная «нарезка», поэтому LHM раскладывает это на пять Сфер: Познание, Среда, Сила, Ресурсы, Социум.

SDT/PENS в этом месте решает другую задачу. Она хорошо объясняет три базовые потребности на уровне переживания — почему игроку становится «хорошо». LHM опирается на эту логику в фундаменте выше по тексту. А здесь нам важно связать запрос игрока с типом игрового опыта и конкретными механиками, поэтому и нужна карта областей жизни.

Почему не три?

Потому что три группы хороши для разговора на уровне «мне в целом не хватает роста / связи / опоры». Для проектирования механик нужно точнее. Ощущение «я могу» может рождаться от решения задач (Познание) и от победы в борьбе (Сила). Ощущение безопасности может быть про пространство (Среда) и про запасы (Ресурсы). Если склеить эти вещи, модель теряет предсказательность: разные игры начинают выглядеть одинаково, хотя на деле они закрывают разные запросы.
...
>
> LHM раскладывает эти группы на пять Сфер как на более удобный уровень детализации: достаточно точно для проектирования, но достаточно просто, чтобы оставаться инструментом, а не энциклопедией.
...
> [!NOTE]
> Важно: здесь речь идет именно о физической энергии («батарейка» тела). Когнитивная энергия (внимание, ментальный ресурс) относится к Сфере Познания.
...
| --- | --- | --- | --- |
| ПОЗНАНИЕ (разум) | **Структура**<br><em>Собрать или разобрать (разгадать)</em> | Эффективность (стабилизация)<br>Шанс (стимуляция), Сила (стимуляция) | <em>Отказ от логики, “меньше думай — больше стреляй”, разгрузка от чрезмерного обдумывания.</em> |
| СРЕДА (окружение) | **Границы**<br><em>Закрыться в доме или открыть карту</em> | Шанс (стабилизация)<br>Эффективность (стабилизация) | <em>Превращение природы в цех. Победа над хаосом окружения через чертежи и т.п.</em> |
| РЕСУРСЫ (активы) | **Эффективность**<br><em>Тайкуны, настройка потоков для получения ресурсов.</em> | Шанс (стимуляция)<br>Социум (стабилизация) | <em>Гача, монетизация дружбы, покупка лояльности и любви.</em> |
| СИЛА (уважение и иерархия) | **Сила**<br><em>Прямая агрессия, доминирование, бросание вызова.</em> | Границы (стабилизация)<br>Структура (стабилизация) | <em>Война как шахматы, убийство без ярости и рефлексов, через логику и мышление.</em> |
| СОЦИУМ (связи и люди) | **Социум**<br><em>Кооп, серверы для отыгрыша роли (RP), эмпатия NPC, а также манипуляции, обман, мафия-подобные игры.</em> | Сила (стабилизация)<br>Эффективность (стабилизация) | <em>Геймификация отношений, дружбы, любви через прогресс. Дейтинг-симы, адалт-игры.</em> |
...
## Справочник игровых механик → глаголы LHM
...
- Ресурсы: сон/энергия (физический тонус). Чем хуже восстановление, тем чаще тянет к Убежищу (Стабилизаторы).
...
Всё, что описано выше в модели, относится к **Прямой Агентности** — когда игрок управляет игрой сам: нажимает кнопки, влияет на исходы, подбирает оптимальные цифры. Игрок тратит свою энергию, преодолевает сопротивление системы и получает дофамин от личной компетентности.

**Викарная Агентность** работает через механизм зеркальных нейронов и психологическую идентификацию — симуляция успеха через Героя-прокси. Игрок инвестирует эмпатию в Аватара, Аватар тратит *свою* энергию, а игрок получает результат «бесплатно» (с точки зрения физических усилий), но присваивает его себе.
...
> на основе работ Cynthia Hoffner (1996), Przybylski, Weinstein, Ryan, & Rigby (2012)
> Они доказали, что игроки получают кайф, когда геймплей позволяет им приблизиться к их идеальному Я.
...
> на основе работ Hoffner (1996) и теория Maccoby & Wilson (1957)
...
> В своей работе Коэн как раз жестко разделяет Идентификацию (я становлюсь им) и Парасоциальное взаимодействие (я общаюсь с ним / наблюдаю за ним как за другом). Коэн назвал бы это отсутствием идентификации в пользу симпатии или парасоциальной привязанности.
...
Это инструмент **Структурирования опыта**. Игрок получает мудрость и катарсис, наблюдая за чужими ошибками.
*Примеры:* The Last of Us Part II, Spec Ops: The Line, Arthur Morgan (RDR2).

### Нарративные Инструменты LHM (Справочник)
...
Частая ошибка — думать, что Геймплей и Сюжет должны всегда говорить одно и то же. В LHM они могут работать в паре, закрывая **разные** дефициты (Принцип разделения труда) или работая в противофазе.
...
Эта игра — идеальный пример того, как LHM объясняет «необъяснимое». Многие считают её интерактивной книгой, но с точки зрения нашей модели это чистейшая **Игра-Тренажёр**.
...
Как выбрать правильный сюжет для вашей игры? В LHM жанр истории определяется не литературными канонами, а ее функцией в общей системе.
...
> Если игроки скипают сюжет, проверьте две причины по LHM:
